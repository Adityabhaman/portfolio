function animation(){
  let canvas, ctx, w, h, thunder, text, particles, animationStarted = false, animationId;

// Constructor for Thunder effect
...
// Constructor for Particles effect
...
// Constructor for Text effect
function Text(options) {
  options = options || {};
  const pool = document.createElement("canvas");
  const buffer = pool.getContext("2d");
  pool.width = w;
  buffer.fillStyle = "#"; // Fill color for the text canvas
...
function loop() {
  update();
  render();
  animationId = requestAnimationFrame(loop); // Store the animation frame ID
}
...
// Update function to manage animations
function update() {
  thunder.forEach((t, i) => t.update(i, thunder));
  particles.forEach((p) => p.update());
  text.update();
}

// Render function to draw on canvas
function render() {
  ctx.clearRect(0, 0, w, h); // Clear the canvas
  ctx.fillStyle = "#0000000"; // Set background to white
  ctx.fillRect(0, 0, w, h); // Fill background
  thunder.forEach((t) => t.render(ctx)); // Render thunder
  particles.forEach((p) => p.render(ctx)); // Render particles
  text.render(ctx); // Render text
}
